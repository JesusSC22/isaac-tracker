...
the `collectibles = { ... }` block in each, and writes a single Python dict
to `tracker/data/eid_descriptions.py`.
...
def parse_eid_file(path: Path) -> dict[int, dict[str, str]]:
    ...
    declaration. Only entries whose host table is in the collectibles allowlist
    are emitted. This avoids needing balanced-brace block extraction (EID lua
    ...
    for m in _ENTRY_RE.finditer(text):
        ...
        if host not in _COLLECTIBLE_TABLE_NAMES:
            continue
...
def write_eid_py(merged: dict[int, dict[str, str]], path: Path) -> None:
    lines = [
...
def main() -> int:
    ...
    for f in files:
        if not f.exists():
            ...
            continue
        partial = parse_eid_file(f)
        merged.update(partial)
        print(f"[build_eid] {f.parent.name}/{f.name}: {len(partial)} entries")

    print(f"[build_eid] {len(merged)} unique collectibles after merge")

    project_root = Path(__file__).resolve().parents[1]
    out_py = project_root / "tracker" / "data" / "eid_descriptions.py"
    write_eid_py(merged, out_py)
    print(f"[build_eid] wrote table -> {out_py}")
    return 0
