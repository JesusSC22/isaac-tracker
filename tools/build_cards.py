...
def parse_pocketitems(
    ...
    for elem in root:
        ...
        # We also keep the raw `achievement_id` (int) so the runtime parser can
        # decide whether the card is unlocked by inspecting the achievements
        # chunk directly (Isaac's save has no per-card 'seen' bitfield).
...
def write_cards_py(cards: list[dict], path: Path) -> None:
    ...
    lines = [
        ...
        "from __future__ import annotations",
        "",
        "# id -> {name (English V1), sprite, removed, desc_es, group, unlock, achievement_id}",
        ...
        "# `achievement_id` is the byte index in the save's achievements chunk that,",
        "# when set to 1, means this card is unlocked (= available in the pool). None",
        "# means the card is unlocked from the start. The Isaac save has no per-card",
        "# 'seen' bitfield, so this is our source of truth for 'do I have this card?'.",
        "CARDS: dict[int, dict] = {",
    ]
    for c in cards:
        ...
        lines.append(
            ...
            f"'group': {c['group']!r}, "
            f"'unlock': {c['unlock']!r}, "
            f"'achievement_id': {c['achievement_id']!r}"
            f"}},"
        )
    lines.append("}")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
